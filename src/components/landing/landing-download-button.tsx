...
import { CheckCircle2, Download, LoaderCircle, PartyPopper, X } from "lucide-react";
...
export function LandingDownloadButton({ source, className, ariaLabel, children }: Props) {
  ...
  return <>
    <button type="button" aria-label={ariaLabel} className={className} onClick={() => { setSuccess(false); setDownloadUrl(""); setError(""); setOpen(true); }}>{children}</button>
    {open ? <div className="fixed inset-0 z-[100] grid place-items-center bg-[#061c14]/75 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget && !pending) setOpen(false); }}>
      <section role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={descriptionId} className="relative w-full max-w-md rounded-3xl bg-[#f6f4ec] p-6 text-left text-[#0a2a1e] shadow-2xl sm:p-8">
        ...
        {success ? <div className="py-3 text-center">
          ...
          <p className="mt-5 flex items-center justify-center gap-2 font-mono text-xs font-bold uppercase tracking-wider text-[#2fa85a]"><PartyPopper className="size-4" /> ¡Descarga iniciada!</p>
          <h2 id={titleId} className="mt-3 text-3xl font-black">¡Felicitaciones!</h2>
          <p id={descriptionId} className="mt-3 text-sm leading-6 text-[#0a2a1e]/65">Tu prueba gratuita de Tienda360 ya está descargándose. Tenés 3 días para probar todas las funciones con tu comercio real.</p>
          <p className="mt-4 rounded-xl bg-[#fce7c4] px-4 py-3 text-sm font-bold">¡Mucho éxito en esta nueva etapa de tu negocio!</p>
          <div className="mt-6 grid gap-3">
            ...
            {downloadUrl ? <a href={downloadUrl} download="Tienda360.zip" className="text-sm font-bold text-[#2fa85a] hover:underline">¿No comenzó? Descargar nuevamente</a> : null}
