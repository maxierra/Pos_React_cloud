create table if not exists public.quick_sale_categories (
  ...
  constraint quick_sale_categories_business_name_key unique (business_id, name)
);

create index if not exists quick_sale_categories_business_sort_idx
  on public.quick_sale_categories (business_id, active desc, sort_order asc, name asc);

alter table public.quick_sale_categories enable row level security;

drop policy if exists quick_sale_categories_all on public.quick_sale_categories;
create policy quick_sale_categories_all on public.quick_sale_categories
for all to authenticated
using (public.is_business_member(business_id))
with check (public.is_business_member(business_id));

create or replace function public.ensure_quick_sale_categories(p_business_id uuid)
returns void
language plpgsql
security definer
...
begin
  ...
  insert into public.quick_sale_categories (business_id, name, active, sort_order)
  values
    (p_business_id, 'Verduleria', true, 0),
    (p_business_id, 'Panaderia', true, 1),
    (p_business_id, 'Carnes', true, 2),
    (p_business_id, 'Fiambreria', true, 3)
  on conflict (business_id, name) do nothing;
end;
$$;

grant execute on function public.ensure_quick_sale_categories(uuid) to authenticated;
grant select, insert, update, delete on public.quick_sale_categories to authenticated;
